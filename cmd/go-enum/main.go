...
func main() {
	var (
		...
		stringFuncName = kingpin.Flag("string-func-name", "Replace stringer function.").Default("").String()
	)
	...
	e := enum.New(*fileName, *trimPrefix, *trimSuffix, *lineNum, *json, *value, formatFuncs[*formatFunc], *all, *stringFuncName)
